...
#define YIBIAOPAN_MODEL_ENABLE 1
#if YIBIAOPAN_MODEL_ENABLE
#include "pbrModel.h"
...
#if YIBIAOPAN_MODEL_ENABLE
    std::shared_ptr<PbrPass> mPbrPass; // PBR渲染管线
...
bool Application::initialize(const XrInstance instance, const XrSession session) {
    ...
    Mymodel.loadFbModel("123");
    Mymodel.loadModel("model/backpack/backpack.obj");
...
//    YIBIAOPANModel->activeMeshTexture("backpack", "models/backpack/ao.jpg"       );
    ...
    mPbrPass->initialize(YIBIAOPANModel->getMeshes());

    mEquirectangularToCubemapPass = std::make_shared<EquirectangularToCubemapPass>();
    mEquirectangularToCubemapPass->initialize(YIBIAOPANModel->getMeshes());

    mIrradiancePass = std::make_shared<IrradiancePass>();
    mIrradiancePass->initialize(YIBIAOPANModel->getMeshes());

    mPrefilterPass = std::make_shared<PrefilterPass>();
    mPrefilterPass->initialize(YIBIAOPANModel->getMeshes());

    mBrdfPass = std::make_shared<BrdfPass>();
    mBrdfPass->initialize(YIBIAOPANModel->getMeshes());
    ...
    std::vector<std::string> passOrder = {"equirectangularToCubemap", "irradiance","prefilter","brdf","pbr"};
...
//PbrModel pbrModel("name");
//pbrModel.loadFbModel("123");
#if YIBIAOPAN_MODEL_ENABLE
...
//    if(isInit){
        isInit = false;
//    }
    Mymodel.render(project, view, model);
}
