...
//        Format("检测到 %d 个Aruco,有 %d 个符合ID: %d, 将采用第 %d 个结果", markerIds.size(),matched_cnt,aruco_id,id_matched_index);
//        if(matched_cnt>1) warnf(  Format("检测到 %d 个Aruco,有 %d 个符合ID: %d, 将采用第 %d 个结果",markerIds.size(),matched_cnt,aruco_id,id_matched_index).c_str());
        if (matched_cnt > 1) {
            warnf("检测到 %zu 个Aruco,有 %d 个符合ID: %d, 将采用第 %d 个结果",
                  markerIds.size(), matched_cnt, aruco_id, id_matched_index);  // 大多数 printf 风格日志函数都支持这样传
        }
