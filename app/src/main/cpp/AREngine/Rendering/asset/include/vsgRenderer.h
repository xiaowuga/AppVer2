...
public:
    ...
    void initRenderer(std::string engine_path, std::vector<vsg::dmat4>& model_transforms, std::vector<std::string>& model_paths, std::vector<std::string>& instance_names, vsg::dmat4 plane_transform)
    {
        ...
        if (numLights >= 1)
        {
            ...
            directionalLight->direction = vsg::normalize(vsg::vec3(0, 1, 0));
        ...
        std::cout << "viewport size :" << cadWindowTraits->width << cadWindowTraits->height;
        ...
        std::cout << "viewport size :" << render_width << render_height;
