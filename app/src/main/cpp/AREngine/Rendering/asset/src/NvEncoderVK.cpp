...
NvEncoderVK::NvEncoderVK(CUcontext cuContext, uint32_t nWidth, uint32_t nHeight, NV_ENC_BUFFER_FORMAT eBufferFormat,
...
NvEncoderVK::~NvEncoderVK()
{
    ReleaseCudaResources();
}

void NvEncoderVK::AllocateInputBuffers(int32_t numInputBuffers)
{
    ...
    m_numInputBuffers = numInputBuffers;
}

void NvEncoderVK::ReleaseInputBuffers()
{
    // Since no input buffers were allocated, we're not going to free them.
}

void NvEncoderVK::RegisterInputResources(std::vector<void*> inputframes,
...
void NvEncoderVK::UnregisterInputResources()
{
    NvEncoder::UnregisterInputResources();
}

void NvEncoderVK::ReleaseCudaResources()
{
    if (!m_hEncoder)
    {
        return;
