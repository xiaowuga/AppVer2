...
void RenderingEngine::init(const Matx33f& K, int width, int height, float zNear, float zFar, int numLevels, std::string shader_path)
{
    this->width = width;
    this->height = height;
    ...
    // std::string dir_path = full_path.parent_path().u8string();
    // if(!dir_path.empty() && dir_path.back()!='/'){
    //     dir_path += '/';
    // }
    // shaderFolder = QString::fromUtf8(dir_path.c_str());
    // std::string dir_path = getExecutablePath()+"/";
    // shaderFolder = QString::fromUtf8(dir_path.c_str());
    std::cout << "Loading tracking shader from: "<<shader_path<<std::endl;
    shaderFolder = QString::fromUtf8(shader_path.c_str());
...
void RenderingEngine::renderSilhouette(vector<Model*> models, GLenum polyonMode, bool invertDepth, const std::vector<cv::Point3f>& colors, bool drawAll)
{
    ...
    for(int i = 0; i < models.size(); i++)
    {
        ...
        if(model->isInitialized() || drawAll)
        {
            ...
            // std::cout << "ObjectTracking pose: " << pose << std::endl;
            // std::cout << "ObjectTracking normalization: " << normalization << std::endl;
            // std::cout << "ObjectTracking pose*normalization: " << pose*normalization << std::endl;
...
void RenderingEngine::renderShaded(vector<Model*> models, GLenum polyonMode, const std::vector<cv::Point3f>& colors, bool drawAll)
{
    ...
    for(int i = 0; i < models.size(); i++)
    {
        ...
        if(model->isInitialized() || drawAll)
        {
            ...
            Matx44f modelViewProjectionMatrix = projectionMatrix*modelViewMatrix;
            std::cout << "ObjTracking pose: " << pose << std::endl;
            std::cout << "ObjTracking normalization: " << normalization << std::endl;
            std::cout << "ObjTracking pose*normalization: " << pose*normalization << std::endl;
            std::cout << "ModelView: " << modelViewMatrix << std::endl;

            std::cout << "pose in render " << pose(0,3) << " " << pose(1,3) << " " << pose(2,3) << "\n";
