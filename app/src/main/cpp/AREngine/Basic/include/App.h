...
public:
    ...
    /*设置RPC发送队列中缓存帧的最大数量，以避免发送队列阻塞
    * 如果发送队列中的缓存帧数量大于maxBufferedPostedFrames-1，则当前帧的CollectRemoteProcs将不会被调用，相关命令因此也不会被执行
    * 如果maxBufferedPostedFrames<=0，则每一帧数据都将被上传，但网络不佳或者帧频较高时会导致阻塞，增加时延
    * 默认值maxBufferedPostedFrames==2
    */
