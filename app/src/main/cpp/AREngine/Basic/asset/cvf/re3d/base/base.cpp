...
{
	auto tempDir = this->getTempDir();
	return tempDir.empty() ? "" : tempDir + "/owners/" + owner + "/";
}

static std::string _getModelDataDir(const ModelInfos& modelInfo)
{
	auto ownerDataDir = app()->getOwnerDataDir(modelInfo.owner);

	if (!ownerDataDir.empty())
		return ownerDataDir + "/model_data/" + modelInfo.name + "/";

	return ff::GetDirectory(modelInfo.modelFile) + "/" + modelInfo.name + "_data/";
}
...
ModelInfos  App::configModel(const std::string& modelFile, const std::string& owner, const std::string& name)
{
	...
	std::string myBufDir = _getModelDataDir(mi);
#if 0
	if (!ff::pathExist(myBufDir))
	{
		ff::makeDirectory(myBufDir);
